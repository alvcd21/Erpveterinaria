...
const Inventory: React.FC = () => {
  ...
  const uniqueMarcas = useMemo(() => [...new Set(phones.map(p => p.marca).filter(Boolean))].sort(), [phones]);
  const uniqueModelos = useMemo(() => {
    const marcaFiltro = phoneForm.marca?.trim().toLowerCase();
    const modelos = phones
      .filter(p => !marcaFiltro || p.marca?.toLowerCase() === marcaFiltro)
      .map(p => p.modelo)
      .filter(Boolean);
  ...
  const handleAiSuggestPrice = async () => {
    ...
    setAiPrice(null);
    try {
      const result = await AIService.suggestPrice(phoneForm.modelo, phoneForm.precioCompra);
      setAiPrice(result?.error ? { error: true } : result);
    } catch {
      setAiPrice({ error: true });
    } finally {
          ...
          <div className={`bg-white rounded-3xl w-full ${activeTab === 'TELEPHONES' || activeTab === 'STOCK' ? 'max-w-4xl' : 'max-w-md'} shadow-2xl p-0 overflow-hidden animate-fade-in flex flex-col max-h-[95vh]`}>
             ...
             <div className="flex-1 overflow-y-auto p-8 bg-slate-50/30 custom-scrollbar">
               <form onSubmit={handleSubmit} className="space-y-6">
                {activeTab === 'TELEPHONES' && (
                    <div className="space-y-6"><div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm"><h4 className="text-xs font-black text-indigo-600 uppercase mb-4 tracking-widest flex items-center gap-2"><Tag size={16}/> Identificadores Únicos</h4><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><label className="text-[10px] font-black text-slate-500 uppercase mb-1.5 block ml-1">IMEI 1 (Principal)</label><input required className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-mono text-sm" value={phoneForm.imei1 || ''} onChange={e => setPhoneForm({...phoneForm, imei1: e.target.value})} placeholder="0000..." /></div><div><label className="text-[10px] font-black text-slate-500 uppercase mb-1.5 block ml-1">IMEI 2 (Opcional)</label><input className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-mono text-sm" value={phoneForm.imei2 || ''} onChange={e => setPhoneForm({...phoneForm, imei2: e.target.value})} /></div></div></div><div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm"><h4 className="text-xs font-black text-indigo-600 uppercase mb-4 tracking-widest flex items-center gap-2"><Smartphone size={16}/> Dispositivo y Precios</h4><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><label className="text-[10px] font-black text-slate-500 uppercase mb-1.5 block ml-1">Marca</label><input required list="marcas-list" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold" value={phoneForm.marca || ''} onChange={e => setPhoneForm({...phoneForm, marca: e.target.value, modelo: ''})} placeholder="Escribe o selecciona..." /><datalist id="marcas-list">{uniqueMarcas.map(m => <option key={m} value={m}/>)}</datalist></div><div><label className="text-[10px] font-black text-slate-500 uppercase mb-1.5 block ml-1">Modelo</label><input required list="modelos-list" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold" value={phoneForm.modelo || ''} onChange={e => setPhoneForm({...phoneForm, modelo: e.target.value})} placeholder="Escribe o selecciona..." /><datalist id="modelos-list">{uniqueModelos.map(m => <option key={m} value={m}/>)}</datalist></div><div><label className="text-[10px] font-black text-slate-500 uppercase mb-1.5 block ml-1">P. Compra</label><input required type="number" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={phoneForm.precioCompra || ''} onChange={e => setPhoneForm({...phoneForm, precioCompra: Number(e.target.value)})} /></div><div><div className="flex items-center justify-between mb-1.5 ml-1"><label className="text-[10px] font-black text-slate-500 uppercase">P. Venta</label>{phoneForm.modelo && phoneForm.precioCompra ? (<button type="button" onClick={handleAiSuggestPrice} disabled={aiPriceLoading} className="text-[9px] font-black text-indigo-500 hover:text-indigo-700 disabled:opacity-50 flex items-center gap-0.5 transition-colors">{aiPriceLoading ? <span className="inline-block w-2 h-2 border border-indigo-500 border-t-transparent rounded-full animate-spin mr-0.5"/> : '💡'} Sugerir precio</button>) : null}</div><input required type="number" className="w-full p-3 bg-emerald-50 border border-emerald-100 rounded-xl text-sm font-black text-emerald-700" value={phoneForm.precioVenta || ''} onChange={e => setPhoneForm({...phoneForm, precioVenta: Number(e.target.value)})} />{aiPrice && !aiPrice.error && aiPrice.precioSugerido != null && (<div className="mt-1.5 flex items-center gap-2 flex-wrap"><span className="text-[10px] text-slate-500">Sugerido: <strong className="text-emerald-700">L. {Number(aiPrice.precioSugerido).toLocaleString()}</strong>{aiPrice.margenEsperado ? ` (margen ${aiPrice.margenEsperado}%)` : ''}</span><button type="button" onClick={() => setPhoneForm(f => ({...f, precioVenta: aiPrice.precioSugerido}))} className="text-[9px] px-1.5 py-0.5 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded transition-all">Aplicar</button></div>)}{aiPrice?.error && <p className="text-[9px] text-slate-400 italic mt-1">IA no disponible.</p>}</div></div></div><div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm"><h4 className="text-xs font-black text-indigo-600 uppercase mb-4 tracking-widest flex items-center gap-2"><MapPin size={16}/> Ubicación y Proveedor</h4><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div><label className="text-[10px] font-black text-slate-500 uppercase mb-1.5 block ml-1">Ubicación</label><select required className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={phoneForm.idubicacion || ''} onChange={e => setPhoneForm({...phoneForm, idubicacion: e.target.value})}><option value="">Seleccionar...</option>{locations.map(l => (<option key={l.idUbicacion} value={l.idUbicacion}>{l.nombre}{l.estante ? ` — Mueble: ${l.estante}` : ''}{l.nivel ? ` | Nivel: ${l.nivel}` : ''}</option>))}</select></div><div><label className="text-[10px] font-black text-slate-500 uppercase mb-1.5 block ml-1">Proveedor</label><select required className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm" value={phoneForm.codProveedor || ''} onChange={e => setPhoneForm({...phoneForm, codProveedor: e.target.value})}><option value="">Seleccionar...</option>{providers.map(p => (<option key={p.codProveedor} value={p.codProveedor}>{p.nombre}</option>))}</select></div></div></div></div>
                )}
