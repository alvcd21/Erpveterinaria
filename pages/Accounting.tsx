...
const Accounting: React.FC = () => {
  ...
  const saveEditTx = async () => {
      ...
      try {
          ...
          setEditingTx(null);
          loadData();
          Swal.fire('Actualizado', 'La rentabilidad ha sido recalculada automáticamente.', 'success');
    ...
    <div className="space-y-6 h-full flex flex-col pb-10">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-4">
                ...
                <div><h2 className="text-xl font-bold text-slate-800">Contabilidad Gerencial</h2><p className="text-xs text-slate-500 font-medium">Rentabilidad Neta y Reparto de Socios</p></div>
            </div>
            <div className="flex flex-wrap gap-2">
                {[{ id:'SUMMARY', label:'Ganancias', icon:<TrendingUp size={16}/> }, { id:'TRANSACTIONS', label:'Auditoría', icon:<Activity size={16}/> }].map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2 transition-all ${activeTab === tab.id ? 'bg-slate-800 text-white shadow-lg' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{tab.icon} {tab.label}</button>
                ))}
                <button onClick={exportPDF} className="px-4 py-2 bg-indigo-600 text-white rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg shadow-indigo-600/20"><Download size={16}/> Exportar PDF</button>
            </div>
            <div className="bg-indigo-50 p-2 rounded-xl border border-indigo-100 flex items-center gap-2">
                <Calendar size={18} className="text-indigo-600 ml-1"/><input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="bg-transparent text-sm font-bold text-indigo-700 outline-none"/>
            </div>
        </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                        <div className="lg:col-span-3 bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden shadow-2xl">
                             <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8">
                                <div><p className="text-indigo-300 text-[10px] font-black uppercase tracking-widest mb-1">Ventas del Día</p><h3 className="text-4xl font-black">L. {report.daily.ingresos.toLocaleString()}</h3><p className="text-emerald-400 text-xs mt-2 font-bold flex items-center gap-1"><ArrowUpRight size={14}/> Util. Bruta: L. {report.daily.utilBruta.toLocaleString()}</p></div>
                                <div className="border-l border-white/10 pl-8"><p className="text-red-300 text-[10px] font-black uppercase tracking-widest mb-1">Gastos Operativos (Negocio)</p><h3 className="text-3xl font-bold text-red-200">L. {report.daily.gastosGral.toLocaleString()}</h3><p className="text-slate-400 text-[10px] mt-2">Deducciones generales compartidas</p></div>
                                <div className="border-l border-white/10 pl-8 bg-emerald-500/10 rounded-2xl p-4"><p className="text-emerald-400 text-[10px] font-black uppercase tracking-widest mb-1">Utilidad Repartible</p><h3 className="text-4xl font-black text-emerald-400">L. {report.daily.utilNetaNegocio.toLocaleString()}</h3><p className="text-[10px] text-emerald-200/50 mt-1">Sujeta a repartición y deducción personal</p></div>
                             </div>
                        </div>
                        <div className="bg-white border-2 border-indigo-100 rounded-3xl p-6 flex flex-col justify-center items-center text-center shadow-sm">
                            ...
                            <p className="text-xs font-bold text-slate-400 uppercase">Inversión Stock</p>
                            <h3 className="text-2xl font-bold text-slate-800 mt-1">L. {report.daily.inversion.toLocaleString()}</h3>
                            <p className="text-[9px] text-slate-400 mt-2 px-4 leading-tight italic">Dinero retirado para reponer inventario. No resta ganancia neta.</p>
                        </div>
                    </div>
                            ...
                            <div key={i} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm relative group hover:border-indigo-400 transition-all">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 mt-4 pt-2 border-t border-slate-50">
                                        <div className="text-center bg-slate-50 p-2 rounded-lg"><p className="text-[9px] text-slate-400 uppercase">Mensual</p><p className="text-xs font-bold text-slate-700">L.{d.gananciaMes.toLocaleString()}</p></div>
                                        <div className="text-center bg-slate-50 p-2 rounded-lg"><p className="text-[9px] text-slate-400 uppercase">Anual</p><p className="text-xs font-bold text-slate-700">L.{d.gananciaAnio.toLocaleString()}</p></div>
                                    </div>
                                </div>
                            </div>
                        ))}
        ...
        {editingTx && (
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
                <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl p-6 animate-fade-in">
                    <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Edit2 className="text-indigo-600" size={20}/> Ajuste de Auditoría</h3><button onClick={() => setEditingTx(null)}><X className="text-slate-400"/></button></div>
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Monto L.</label><input type="number" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none font-bold text-slate-800" value={editForm.monto} onChange={e => setEditForm({...editForm, monto: e.target.value})} /></div>
                            {editingTx.tipo === 'INGRESO' && <div><label className="text-[10px] font-bold text-slate-400 uppercase mb-1 block">Costo L. (Inversión)</label><input type="number" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none" value={editForm.costo} onChange={e => setEditForm({...editForm, costo: e.target.value})} /></div>}
                        </div>
                        {editingTx.tipo === 'EGRESO' && (
                            <div className="space-y-4 bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100">
                                <div>
                                    <label className="text-[10px] font-bold text-indigo-400 uppercase mb-2 block">Categoría de Gasto</label>
                                    <select className="w-full p-3 bg-white border border-slate-200 rounded-xl text-xs font-bold" value={editForm.categoria} onChange={e => setEditForm({...editForm, categoria: e.target.value})}>
                                        <option value="Gasto Operativo">Gasto Operativo (Resta Ganancia)</option>
                                        <option value="Compra de Producto">Compra Stock / Mercancía (Reinversión)</option>
                                        <option value="Otros">Otros</option>
                                    </select>
                                </div>
                                <div>
                                    <label className="text-[10px] font-bold text-indigo-400 uppercase mb-2 block">Asignar a Socio (Deducción Personal)</label>
                                    ...
                                    <p className="text-[9px] text-slate-400 mt-2 italic">* Si selecciona un socio, este gasto solo se le descontará a él.</p>
                                </div>
                            </div>
                        )}
                    </div>
                    <div className="flex gap-3 mt-8"><button onClick={() => setEditingTx(null)} className="flex-1 py-3 bg-slate-100 text-slate-500 font-bold rounded-xl">Cancelar</button><button onClick={saveEditTx} className="flex-1 py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-lg">Guardar Cambios</button></div>
                </div>
            </div>
        )}
    </div>
